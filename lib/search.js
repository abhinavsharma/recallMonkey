...
Search.prototype.createIDFMap = function(tokens) {
  ...
  let outerSelections = ["COUNT(1) as count"];
  for (let i = 0; i < tokens.length; i++) {
    params["left"+i]  = '% ' + tokens[i]; + '%';
    params["right"+i] = '%'  + tokens[i]  + ' %';
    params["token"+i] = tokens[i];
    innerSelections.push("(title LIKE :left" + i + 
                    " OR title LIKE :right" + i + 
                    " OR LOWER(title) = :token" + i + ") as match" + i);
    innerConditions.push("(title LIKE :left" + i + 
                    " OR title LIKE :right" + i + 
                    " OR LOWER(title) = :token)");
  ...
  let inner = "(SELECT " + innerSelections + " FROM moz_places WHERE " +
              innerConditions + ")";
