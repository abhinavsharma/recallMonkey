 ...
 * @return undefined
 */
Search.prototype.createIDFMap = function(tokens, cont) {
  let me = this;
  let idfMap = {};
  ...
  function showIDFMap() {
    reportError("YAY");
    reportError(J(me.idfMap));
  }

  stmt.executeAsync({
    handleResult: function(aResultSet) {
      reportError("HANDLING RESULT");
      for (let row = aResultSet.getNextRow(); row; row=aResultSet.getNextRow()) {
        reportError("HANDLING ROW");
        for (let i = 0; i < tokens.length; i++) {
          reportError("HANDLING ROW+TOKEN" + tokens[i]);
  ...
  */
  ...
  for (let i = 0; i < tokens.length; i++) {
    //let n = d["count" + i];
    //let N = d.count;
    me.idfMap[tokens[i]] = 1//Math.log((N - n + 0.5)/(n +0.5));
  }
}

Search.prototype.showIDFMap = function() {
  let me = this;
  reportError(J(me.idfMap));
}
...
Search.prototype.search = function(query, params, data, worker) {
  ...
  function cont() {
    me.queryTable(tokens, params.preferredHosts, 
                  params.excludedHosts,
                  params.timeRange, 
                  params.limit, 
                  params.skip, 
                  params.prioritizeBookmarks,
                  data,
                  worker);
     }
 
  me.createIDFMap(tokens, cont);
}
