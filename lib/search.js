...
Search.prototype.queryTable = function(words, preferredHosts, excludedHosts, timeRange, limit, skip, bookmark) {
  let me = this;
  me.createIDFMap(words);
  let wordSelections = [];
  let wordConditions = [];
  let rankSelection = [];
  let names = ["id", "title", "url", "frecency", "score", "rev_host", "last_visit_date", "is_bookmark"];
  let wordParams = {};
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    //wordSelections.push("(tags LIKE :exact" + i  + ") as url_" + i);
  ...
  let order = ' ORDER BY is_bookmark DESC, score DESC, frecency DESC LIMIT ' + skip + "," + limit;
  ...
  let inner = "(SELECT id, title, url, frecency, rev_host,  is_bookmark, last_visit_date," + selections + 
    " FROM (" + me.inner + ") WHERE " + conditions + ")";
  let query = "SELECT id,title, url, frecency, rev_host,is_bookmark,last_visit_date," + ranked + " FROM " + 
    inner + order;
  reportError(query);
  ...
  return utils.spinQuery(Places.PlacesUtils.history.DBConnection, {
    ...
    "params": wordParams,
    "query" : query,
  }).map(function ({id, title, url, frecency, rev_host, last_visit_date}) {
    return {
      "title" : title,
      "url" : url,
      "revHost": rev_host ,
...
Search.prototype.search = function(query, params) {
  ...
  let result = me.queryTable(words, params.preferredHosts, params.excludedHosts, params.timeRange, params.limit, params.skip);
