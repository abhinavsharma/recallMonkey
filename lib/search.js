...
function Search() {
  ...
  me.inner = "SELECT * FROM " + 
    "(SELECT * FROM " + 
      "(SELECT p.id as id, 1 as is_bookmark, p.title as title,  p.url as url, p.frecency as frecency, p.rev_host as rev_host, p.last_visit_date as last_visit_date, GROUP_CONCAT(tag) as tags FROM " + 
        "(SELECT t.title as tag, b.fk as place_id FROM " + 
          "(SELECT * FROM moz_bookmarks WHERE parent = :rowid) t JOIN " + // select tags
          "(SELECT * FROM moz_bookmarks WHERE type=1) b ON b.parent=t.id" + // join bookmarks with their respective tags
        ") r JOIN moz_places p ON p.id=r.place_id GROUP BY p.id" + // join tagged bookmarks with moz_places for extra info like title, url, etc.
      ") UNION SELECT * FROM " + // we have tagged bookmarks, must union with untagged bookmarks and all other places
      "(SELECT p.id as id, 1 as is_bookmark, b.title as title, p.url as url, p.frecency as frecency, p.rev_host as rev_host, p.last_visit_date as last_visit_date, '' as tags FROM " + 
        "(SELECT * FROM moz_bookmarks WHERE title is not null and fk is not null" + 
        ") b JOIN moz_places p on p.id = b.fk WHERE p.url LIKE 'http%' AND p.title is not null AND p.last_visit_date is not null GROUP BY p.id" + // union with all other bookmarks (maybe untagged)
      ") UNION SELECT id, 0 as is_bookmark, title, url, frecency, rev_host, last_visit_date, '' as tags FROM moz_places WHERE title is not null" + // union with all other places
...
/**
 * Queries the database for the given tokens.
 * 
 * @param {list}    tokens : 
 * @param {list}    preferredHosts : 
 * @param {list}    excludedHosts : 
 ...
 * @return {list}
 */
Search.prototype.queryTable = function(tokens, preferredHosts, excludedHosts, timeRange, limit, skip, prioritizeBookmarks) {
  let me = this;
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    wordParams["submatch" + i] = '%'+ word + '%';
    wordParams["word" + i] = word;
    wordParams["idf" + i] = me.idfMap[word];
    
    /* add relevant selections from this token into the search query */
  ...
  if (timeRange != 0) {
    ...
    strictConditions = t + 
      " - last_visit_date < (:timeRange * 24 * 60 * 60 * 1000 * 1000) AND last_visit_date IS NOT NULL";
...
Search.prototype.search = function(query, params) {
  ...
  reportError("querying " + J(tokens));
  let result = me.queryTable(tokens, params.preferredHosts, params.excludedHosts, params.timeRange, params.limit, params.skip, params.prioritizeBookmarks);
  return result;
}
